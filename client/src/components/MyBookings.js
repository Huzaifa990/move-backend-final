    ...
    return(
        ...
        {name.length>0?name.map((data)=>{
                if(data !== null){
                    return(
                        <>
                        <div className="container pt-5 pb-3" key={data.car?.carName} style={{marginBottom: "50px", padding: "0px 100px"}}>
                    <h1 className="display-4 text-uppercase  mb-3">{data?.car?.company} {data?.car?.carName}</h1>
                    <img className="img-fluid mb-4 car-listings" style={{width: "30%"}} src={data?.car?.picture[0]} alt="car"/>
                    <ul style={{float: "right"}} className="list-inline m-0">
                        ...
                        <li className="list-inline-item">
                            <button className="btn btn-success btn-sm rounded-0 px-4" type="button" data-toggle="tooltip"
                                data-placement="top" title="Edit" onClick={ () => editCar(data?.car?._id) }><i className="fa fa-edit"></i></button>
                        </li>
                        <li className="list-inline-item">
                            <button className="btn btn-danger btn-sm rounded-0 px-4" type="button" data-toggle="tooltip"
                                data-placement="top" title="Delete" onClick={ () => deleteCar(data?.car?._id) }><i className="fa fa-trash"></i></button>
                        </li>
                    </ul>
                    ...
                    <span style={{position: "relative", float: "right", bottom:"180px", right:"0"}}>
                        <h5>PKR {data?.car?.rentPerDay}/Day</h5>
                            ...
                            <br/>
                            <button className="btn btn-primary px-3" onClick={ () => showId(data._id) }>View Booking</button>
                    </span>
